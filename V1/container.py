...
class TreeStyle(JSONVisualizer):
    ...
    def display(self):
        ...
        self._display_recursive(self.data, 0, middle_icon, leaf_icon)

    # 修改递归显示方法以使用图标
    def _display_recursive(self, obj, depth, middle_icon, leaf_icon, is_last=False):
        items = list(obj.items())
        for i, (k, v) in enumerate(items): #处理同级的每一个元素例如orange和apple
            # if not is_last and len(items[i-1])>0: #如果不是最后一个元素且元素个数大于1
            #     prefix = "│   " * depth
            # else:
            #     prefix = "    " * depth
            prefix = "    " * depth
            ...
            if isinstance(v, dict):
                print()  
                self._display_recursive(v, depth + 1, middle_icon, leaf_icon, is_last_item)
...
class LinearStyle(JSONVisualizer):
    ...
    def display(self):
        middle_icon = self.icon_factory.get_middle_icon()
        leaf_icon = self.icon_factory.get_leaf_icon()
        self._display_recursive(self.data, "", middle_icon, leaf_icon)

    def _display_recursive(self, obj, prefix, middle_icon, leaf_icon):
        for k, v in obj.items():
            ...
            if isinstance(v, dict):
                print()  # 换行以便下一级缩进
                self._display_recursive(v, prefix + "│   ", middle_icon, leaf_icon)
            elif isinstance(v, str) and v:
                print(": " + v)
            else:
...
class TreeStyle2(JSONVisualizer):
    ...
    def display(self):
        middle_icon = self.icon_factory.get_middle_icon()
        leaf_icon = self.icon_factory.get_leaf_icon()
        self._display_recursive(self.data, 1 ,middle_icon, leaf_icon)

    
    def _display_recursive(self, obj, depth, middle_icon, leaf_icon, is_last_sibling=False):
        items = list(obj.items())
        for i, (k, v) in enumerate(items):
            ...
            if isinstance(v, dict):
                ...
                self._display_recursive(v, depth + 1,middle_icon,leaf_icon, i == len(items) - 1)
            elif isinstance(v, str) and v:
                print(':',v)
            else:
                print()  # 对于非字典且无值的情况，简单换行
...
class RectangleStyle(JSONVisualizer):
    ...
    def display(self):
        ...
        if max_widths >50:
            max_widths = 50
        # print(max_widths)
        self._display_recursive(self.data, 1 ,middle_icon, leaf_icon, max_widths)

                        
    def _display_recursive(self, obj, depth, middle_icon, leaf_icon, max_width=40):
        items = list(obj.items())
        for i, (key, value) in enumerate(items):
            ...
            if isinstance(value, dict):
                ...
                self._display_recursive(value, depth + 1, middle_icon, leaf_icon, max_width)
